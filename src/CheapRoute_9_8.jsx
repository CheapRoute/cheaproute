...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    if (mustStop) {
      ...
      // How much to fill:
      // - Fastest: always fill full → fewest stops, don't care about price
      // - Cheapest: if price here is lower than next stop, fill as much as possible
      //             if price ahead is lower, buy just enough to get there
      // - Balanced: fill to get comfortably to next stop, bias toward cheap fills
      ...
      if (optimization === "fastest") {
        targetFuel = usableTank; // fill full every time
      } else if (optimization === "cheapest") {
        if (cheaperHere) {
          // Fill as much as possible — cheap now, expensive ahead
          targetFuel = usableTank;
        } else if (cheaperAhead) {
          // Buy minimum — cheaper gas is coming
          targetFuel = minFuelNeeded + 1; // just enough + 1gal safety
        } else {
          targetFuel = minFuelNeeded + (bufferMiles / mpg);
        }
      } else {
        // Balanced: fill generously when cheap, conservatively when expensive ahead
        if (cheaperHere) {
          targetFuel = usableTank * 0.95;
        } else {
          targetFuel = minFuelNeeded + (bufferMiles / mpg) + 2;
        }
      }
...
export default function App() {
  ...
  async function calc(){
    ...
    try {
      ...
      if(route.routeLegs) segs = buildSegsFromLegs(route.routeLegs, vehicle.fuelType, route.totalMiles, practicalRange, prefs.bufferMiles||25, prefs.maxDetour||5, setDebugLog);
