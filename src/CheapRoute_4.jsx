...
async function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange) {
  ...
  for (let i = 0; i < stopPositions.length; i += 5) {
    ...
    console.log("CheapRoute geocoder batch", i, "results:", cities);
    for (let j = 0; j < batch.length; j++) {
      const p = batch[j];
      const geocodedCity = cities[j];
      const fallbackCity = getFallbackCity(p.targetMile, totalMiles, legs);
      console.log(`Mile ${p.targetMile}: geocoded="${geocodedCity}" fallback="${fallbackCity}"`);
...
export default function App() {
  ...
  async function calc(){
    ...
    if(route.routeLegs) segs = await buildSegsFromLegs(route.routeLegs, vehicle.fuelType, route.totalMiles, practicalRange);
