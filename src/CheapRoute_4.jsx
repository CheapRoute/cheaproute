  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <div style={{marginBottom:16,position:"relative"}}>
        <Lbl hint={!data.make?"Select make first":modelQuery.length<2?"Type at least 2 characters to search":"Searching EPA database..."}>
          Model — type to search
        </Lbl>
        <input
          value={modelQuery}
          onChange={e=>handleModelInput(e.target.value)}
          placeholder={data.make?`e.g. RX 350, CR-V, F-150, 4Runner`:"Select make first"}
          disabled={!data.make}
          style={{paddingRight:data.model?"36px":"14px",borderColor:data.model?C.tealBorder:C.border}}
        />
        {data.model&&<span style={{position:"absolute",right:12,top:36,fontSize:14,color:C.teal}}>✓</span>}
