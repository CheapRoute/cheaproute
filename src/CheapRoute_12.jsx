...
function buildSegsFromLegs(legs, fuelType, totalMiles) {
  ...
  // Pull all unique cities from all leg end addresses
  // For a single-leg route, parse the waypoints from the overview path
  const legCities = legs.map(leg => {
    const parts = leg.end_address.split(",");
    // Format: "City, State ZIP, Country" -- take city and state
    const city = parts[0]?.trim() || "";
    const state = parts[1]?.trim().replace(/\d+/g,"").trim() || "";
    return city && state ? `${city}, ${state}` : null;
  }).filter(Boolean);

  // If only one leg (direct route), use our corridor database
  ...
  // Build corridor city list
  const corridorKey = `${oState}-${dState}`;
  const corridorCities = CORRIDOR_CITIES[corridorKey] || [];
  ...
  for (let i = 1; i <= count; i++) {
    ...
    if (legCities.length > 1) {
      // Multi-leg route — use actual leg cities
      const idx = Math.min(Math.floor(frac * legCities.length), legCities.length - 1);
      city = legCities[idx];
    } else if (corridorCities.length > 0) {
      // Single leg — use known corridor cities
      const idx = Math.min(Math.floor(frac * corridorCities.length), corridorCities.length - 1);
      city = corridorCities[idx];
    } else {
      // Last resort — interpolate state from origin/dest
      city = frac < 0.33 ? `${oCity} area, ${oState}` : frac < 0.66 ? `Midpoint, ${oState}` : `${dCity} area, ${dState}`;
    }
    ...
    segs.push({
      city,
      miles: Math.round(frac * totalMiles),
...
const CORRIDOR_CITIES = {
  "NC-CO": ["Knoxville, TN","Nashville, TN","Memphis, TN","Little Rock, AR","Oklahoma City, OK","Amarillo, TX","Albuquerque, NM","Raton, NM","Pueblo, CO"],
  "CO-NC": ["Pueblo, CO","Raton, NM","Albuquerque, NM","Amarillo, TX","Oklahoma City, OK","Little Rock, AR","Memphis, TN","Nashville, TN","Knoxville, TN"],
...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    // Must stop: arrival range below buffer, OR we can't make it to next stop safely
    const mustStop = roa < bufferMiles || ranAfterNext < bufferMiles;
    ...
    if (mustStop) {
      ...
      if (optimization === "fastest") {
        // Fastest: buy just enough to reach next stop with buffer
        gallons = Math.max(0.5, Math.ceil((distNext/mpg + bufferMiles/mpg - fa)*2)/2);
        gallons = Math.min(gallons, usableTank - fa);
      } else {
        // Default: fill up to 80% of usable tank for efficiency
        const targetFuel = usableTank * 0.80;
        gallons = Math.max(0.5, Math.ceil((targetFuel - fa)*2)/2);
        gallons = Math.min(gallons, usableTank - fa);
      }
      action="stop";
  ...
  return { stops, warnings, segs,
    summary:{
      totalMiles,
      totalGallons: Math.round(totalMiles/mpg*10)/10,
      baseline: Math.round(baseline*100)/100,
      optCost: Math.round(optCost*100)/100,
