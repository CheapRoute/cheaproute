...
async function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange, setDebugLog) {
  ...
  // the car arrives at the next candidate with ~60mi to spare — enough slack
  // that buffer changes actually affect stop count. Clamp 150–350 mi.
  const stopInterval = Math.min(350, Math.max(150, (vehicleRange || 300) - 60));

  // Build a point list with cumulative miles AND the geocoded address from each step.
  // Google Directions returns address_components on each leg's start/end — we use
  // the nearest step's via_waypoint or the leg's own geocoded address.
  // This avoids a separate Geocoding API call entirely.
  const routePoints = [];
  let cumMiles = 0;
  ...
  // Extract city/state from a leg by finding the leg whose cumulative start
  // mile is closest to targetMile, then reading its start_address.
  function cityFromLegs(targetMile) {
    // Build cumulative mile markers for each leg boundary
    let legMiles = 0;
    let bestLeg = legs[0];
    let bestDist = Infinity;
    let cumLeg = 0;
    for (const leg of legs) {
      const legLen = leg.distance.value / 1609.34;
      const midMile = cumLeg + legLen / 2;
      if (Math.abs(midMile - targetMile) < bestDist) {
        bestDist = Math.abs(midMile - targetMile);
        bestLeg = leg;
      }
      cumLeg += legLen;
    }
    // Try to parse city, state from the leg's start_address
    // Google format: "123 Street, City, STATE ZIP, USA"
    const addr = bestLeg.start_address || "";
    const parts = addr.split(",").map(s => s.trim()).filter(Boolean);
    // State is typically second-to-last, before "USA"
    let city = "", state = "";
    for (let i = parts.length - 2; i >= 0; i--) {
      const p = parts[i];
      const stateMatch = p.match(/^([A-Z]{2})(?:\s+\d+)?$/);
      if (stateMatch) {
        state = stateMatch[1];
        if (i > 0) city = parts[i - 1].replace(/^\d+\s+/, ""); // strip street number
        break;
      }
    }
  ...
  const segs = [];
  const seen = new Map(); // deduplicate by city name
  ...
  while (nextStopMile < totalMiles - 30) {
    ...
    // Get city: try leg address parsing first, then corridor fallback
    const legCity = cityFromLegs(nextStopMile);
    const fallbackCity = getFallbackCity(nextStopMile, totalMiles, legs);
    const city = legCity || fallbackCity;
    const source = legCity ? "✓ leg addr" : "◆ corridor";
    if (setDebugLog) setDebugLog(prev => [...prev, `Mile ${Math.round(nextStopMile)}: ${source}: ${city}`]);

    // Deduplicate by city — keep the stop closest to the city's true midpoint
    const cityKey = city.toLowerCase().trim();
    const existing = seen.get(cityKey);
    const seg = {
      city,
      miles: Math.round(nextStopMile),
      lat: point.lat,
      lng: point.lng,
    ...
    if (!existing) {
      seen.set(cityKey, seg);
      segs.push(seg);
    }
    // If duplicate city, skip — first occurrence is closest to true entry point
...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    if (mustStop) {
      ...
      // Fill to full on forced stops — partial fills cascade into more forced stops.
      // Optimization mode only affects smart (price-based) stops below.
      const actualTarget = usableTank;
      gallons = Math.ceil((actualTarget - fa) * 2) / 2;
