...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    // When we have enough range to reach the next candidate safely,
    // each mode decides differently whether to stop or push ahead:
    // Fastest: stop whenever cantReachNext (fewest decisions, just keep tank full)
    // Cheapest: skip expensive stops aggressively, only stop when truly forced
    //           or when price is meaningfully cheaper here vs ahead
    // Balanced: skip mildly expensive stops, stop when moderately cheaper here
    // Top Rated: stop at good stations regardless of small price differences
    const canSkipSafely = !mustStop && !cantReachNext;
    if (canSkipSafely && optimization !== "fastest") {
      const pricePremium = priceHere - priceNext; // positive = expensive here vs ahead
      if (optimization === "cheapest" && pricePremium > 0.02) continue;   // skip if >2¢ cheaper ahead
      if (optimization === "balanced" && pricePremium > 0.06) continue;   // skip if >6¢ cheaper ahead
      if (optimization === "toprated" && seg.rating < 3.5 && pricePremium > 0.04) continue;
    }
