...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    if (canSkipSafely || tooSoon) {
      // Always skip if we haven't driven 150mi since last fill (unless dramatically cheaper)
      if (tooSoon) continue;
      if (optimization === "fastest") continue;
      if (optimization === "cheapest") {
        const nextIsCheaper = priceNext < priceHere - 0.01;
        const pricesSimilar = Math.abs(priceHere - priceNext) < 0.04;
        const goodRange = roa > bufferMiles * 3;
        if (nextIsCheaper || (pricesSimilar && goodRange)) continue;
      }
      if (optimization === "balanced") {
        if (priceNext < priceHere - 0.04) continue;
      }
    }

    // Smart stop: worth stopping even when not forced
    // Cheapest: stop early if price here is noticeably cheaper than next (stock up)
    // Balanced: stop early if moderate price advantage
    // Fastest: never smart-stop
    // Top Rated: stop at high-rated stations
    const smartStop =
      optimization === "cheapest"  ? (priceNext > priceHere * 1.03 && roa > bufferMiles * 2) :
      optimization === "fastest"   ? false :
      optimization === "toprated"  ? (seg.rating >= 4.0 && roa > bufferMiles * 2) :
                                     (priceNext > priceHere * 1.04 && roa > bufferMiles * 2);
