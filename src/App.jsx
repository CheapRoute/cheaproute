...
function buildSegsFromLegs(legs, fuelType, totalMiles) {
  // More stops for longer routes — ensure segments never exceed practical range
  const count = Math.max(3, Math.min(12, Math.floor(totalMiles / 60)));
  const segs = [];
...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    // 4. Range getting critically low (within 1.3x buffer) — always stop
    const cantReachNext = ranAfterNext < bufferMiles;
    const criticallyLow = roa < bufferMiles * 1.3;
    const mustStop = roa < bufferMiles || cantReachNext || roa < 0 || criticallyLow;
