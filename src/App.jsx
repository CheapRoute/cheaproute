...
function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange, bufferMiles, maxDetour, realPrices) {
  // Generate candidates every ~100mi so the engine has real choices.
  // Cheapest/Balance modes can then skip cheap-to-reach candidates in favour
  // of cheaper gas further down the road. Fastest mode still fills full and
  // skips as many as possible. Without dense candidates every stop looks the
  // same because there's nothing to compare against.
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    if (mustStop || cantReachNext) {
      ...
      if (optimization === "fastest") {
        ...
        // Cheap here → fill FULL so we skip 2-3 candidates ahead
        // Expensive here → buy bare minimum to reach cheaper gas ahead
        if (cheaperHere || bigSavingHere) targetFuel = usableTank;
        else if (cheaperAhead)            targetFuel = minFuelNeeded + 1.0;
        ...
        // Balanced: fill full when cheap (skip ahead), comfortable fill otherwise
        if (cheaperHere || bigSavingHere) targetFuel = usableTank * 0.95;
        else if (cheaperAhead)            targetFuel = minFuelNeeded + (bufferMiles / mpg) + 1.0;
        else                              targetFuel = minFuelNeeded + (bufferMiles / mpg) + 20/mpg;
      }
