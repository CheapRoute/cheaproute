...
async function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange) {
  // Space candidates at (range - 60) so after a full fill you arrive at the
  // next candidate with ~60 miles of buffer remaining. This means the engine
  // can actually SKIP candidates when the buffer setting is low, making the
  // Fewer/More Stops buttons meaningful. Clamp 150–350 mi.
  const stopInterval = Math.min(350, Math.max(150, Math.round((vehicleRange || 300) - 60)));
  const routePoints = [];
  let cumMiles = 0;
  ...
  for (let i = 0; i < stopPositions.length; i += 5) {
    ...
    for (let j = 0; j < batch.length; j++) {
      const p = batch[j];
      const city = cities[j] || getFallbackCity(p.targetMile, totalMiles, legs);
      segs.push({
        city,
...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    const rangeIsCritical = roa < bufferMiles * 1.5 || ranAfterNext < bufferMiles;
    ...
    // 1. Arrival range is below buffer (user's chosen safety margin)
    // 2. Can't safely reach the next candidate stop
    // 3. Already out of fuel (roa < 0)
    // NOTE: removed criticallyLow/nextWillBeLow at 2x buffer — that threshold
    // fired on every stop for long-range vehicles, making More/Fewer Stops do nothing.
    // A strict buffer check + cantReachNext is sufficient and buffer-responsive.
    const cantReachNext = ranAfterNext < bufferMiles;
    const mustStop = roa < bufferMiles || cantReachNext || roa < 0;
    ...
    if (mustStop) {
      ...
      // Always fill to full on a forced stop — partial fills cause the next
      // candidate to also be a forced stop, creating a cascade of extra stops.
      // Optimization mode only affects SMART stops (price-based), not forced stops.
      const targetFuel = usableTank;
      ...
      const fa = Math.max(0, roa/mpg);
      const smartTarget = optimization === "cheapest" ? usableTank
                        : optimization === "fastest"  ? usableTank * 0.70
                        : usableTank * 0.85;
      gallons = Math.max(3, Math.ceil((smartTarget - fa)*2)/2);
...
// ─── MAP COMPONENT ────────────────────────────────────────────────────────────
function RouteMap({stops,originCoords,destCoords,routeResult}) {
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(maps=>{
      ...
      // Use the exact same DirectionsResult from Step 1 — no second API call,
      // so the map always shows the identical route the user confirmed.
      if (routeResult) {
    ...
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <Card style={{marginBottom:16,background:"#f8fafd"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>updatePref({bufferMiles:Math.max(trip.bufferMiles-15,25),maxDetour:Math.min(trip.maxDetour+3,15)})} style={{flex:1,background:"#f0ebff",border:"1.5px solid #c4b5fd",color:C.purple,borderRadius:8,padding:"9px 8px",fontSize:13,fontWeight:600}}>⬇ Fewer Stops</button>
          <button onClick={()=>updatePref({bufferMiles:Math.min(trip.bufferMiles+25,100),maxDetour:Math.max(trip.maxDetour-3,2)})} style={{flex:1,background:"#fff7ed",border:"1.5px solid #fcd9a0",color:C.amber,borderRadius:8,padding:"9px 8px",fontSize:13,fontWeight:600}}>⬆ More Stops</button>
      ...
      {view==="map"&&(
        <div style={{marginBottom:16}}>
          <RouteMap stops={stops} originCoords={originCoords} destCoords={destCoords} routeResult={trip.routeResult} />
