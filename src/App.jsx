...
function StepVehicle({data,onChange,onNext,onBack,saved,onSave}) {
  ...
  async function pickTrim(opt) {
    ...
    if (specs) {
      // EPA MPG stored separately; real-world is 88% of EPA combined
      const realWorldMpg = String(Math.round(specs.combined * REAL_WORLD_FACTOR * 10) / 10);
      // Tank: use EPA tank size; gallons at empty = listed tank - 2 gal reserve minimum
      const listedTank = specs.tank || 0;
      const usableAtEmpty = listedTank > 2 ? String(Math.round((listedTank - 2) * 10) / 10) : "";
      // Force update all fields so they register in state and enable Save button
      const updates = {
        trim: opt.text,
        ...
        epaOnlyMpg: String(specs.combined),
        mpg: data.mpgLocked ? data.mpg : realWorldMpg,
        listedTank: listedTank ? String(listedTank) : data.listedTank||"",
        usableTank: data.tankLocked ? data.usableTank : usableAtEmpty,
        tankLocked: false,
        mpgLocked: false,
      };
      onChange(updates);
      // Small delay then set again to ensure React state updates
      setTimeout(() => onChange(updates), 100);
      setEpaStatus("found");
  ...
  const ready=data.mpg&&(data.usableTank||data.listedTank)&&data.fuelType;
