...
function getPrice(city, fuelType) {
  ...
  // Apply city-level override if we have one
  const cityLower = (city || "").toLowerCase();
  for (const [key, val] of Object.entries(CITY_PRICE_OVERRIDE)) {
    if (cityLower.includes(key)) { mult = val; break; }
  }

  // Stable station-level variation: ±$0.08 based on city name hash
  // Same city always gets same price — no flickering
...
function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange, bufferMiles, maxDetour, setDebugLog) {
  // Stop exactly at (vehicleRange - bufferMiles) from the last fill.
  // e.g. 378mi range, 25mi buffer → stop at 353mi intervals.
  // Never more than vehicleRange from the last fill (would run out).
  const stopInterval = Math.max(50, vehicleRange - bufferMiles);
  ...
  const HIGHWAY_CITIES = [
    ...
    ["Columbia, MO",38.951,-92.334],["Joplin, MO",37.084,-94.513],
  ...
  function getNearestCity(lat, lng, maxDetourMiles) {
    if (!lat || !lng) return null;
    const maxDeg = maxDetourMiles / 55; // rough degrees (conservative)
    let best = null, bestDist = Infinity;
    for (const [name, clat, clng] of HIGHWAY_CITIES) {
      const d = Math.sqrt((lat-clat)**2 + (lng-clng)**2);
      if (d < bestDist && d < maxDeg) { bestDist = d; best = name; }
    }
    return best;
  }
  ...
  while (nextStopMile < totalMiles - 20) {
    ...
    const nearbyCity = getNearestCity(point.lat, point.lng, maxDetour);
    const city = instrCity || nearbyCity || `Mile ${Math.round(thisMile)}`;
    const source = instrCity ? "✓ directions" : nearbyCity ? "◆ nearby" : "✗ mile marker";

    if (setDebugLog) {
      setDebugLog(prev => [...prev,
        `Mile ${Math.round(thisMile)}: ${source}: ${city} @ (${point.lat?.toFixed(2)},${point.lng?.toFixed(2)})`
      ]);
    }
...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    // ── Per-mode detour enforcement ──────────────────────────────────────────
    // cheapest: willing to go any detour for cheaper gas (ignore detour limit)
    // fastest: strict — never detour at all beyond the limit
    // balanced/toprated: respect detour limit unless range is critical
    const rangeIsCritical = mustStop || cantReachNext;
    if (seg.detourMiles > maxDetour && !rangeIsCritical) {
      if (optimization === "cheapest") {
        // still consider it if price is significantly cheaper
        if (!(pricier && seg.detourMiles <= maxDetour * 3)) continue;
      } else {
        continue; // fastest/balanced/toprated skip over-detour stops
      }
    }
    ...
    const bigSavingHere = priceHere < priceNext * 0.93; // >3% cheaper = worth stocking up

    // Smart stop: worthwhile to stop even though we don't HAVE to
    // Each mode has different thresholds for what counts as "worth it"
    const smartStop =
      optimization === "cheapest"  ? (pricier && roa > bufferMiles) :            // any price rise
      optimization === "fastest"   ? false :                                       // never detour for price
      optimization === "toprated"  ? (seg.rating >= 4.0 && roa > bufferMiles*2) : // high-rated station
                                     (pricier && roa > bufferMiles * 1.5);         // balanced: moderate rise
    ...
    if (mustStop || cantReachNext) {
      ...
      // How much to fill — each mode has distinct strategy:
      let targetFuel;
      if (optimization === "fastest") {
        // Always fill full — minimizes number of stops, price irrelevant
        targetFuel = usableTank;
      } else if (optimization === "cheapest") {
        // Stock up when cheap here, buy minimum when cheaper ahead
        if (bigSavingHere)       targetFuel = usableTank;           // fill full, great price
        else if (cheaperHere)    targetFuel = usableTank * 0.90;    // fill most, decent price
        else if (cheaperAhead)   targetFuel = minFuelNeeded + 1.0;  // bare minimum, better ahead
        else                     targetFuel = minFuelNeeded + (bufferMiles / mpg);
      } else if (optimization === "toprated") {
        // Fill generously at high-rated stations, minimum at low-rated
        targetFuel = seg.rating >= 4.0
          ? usableTank * 0.90
          : minFuelNeeded + (bufferMiles / mpg) + 2;
      } else {
        // Balanced: price-aware but not extreme
        if (cheaperHere)  targetFuel = usableTank * 0.85;
        else              targetFuel = minFuelNeeded + (bufferMiles / mpg) + 3;
      }

      targetFuel = Math.min(usableTank, Math.max(minFuelNeeded, targetFuel));
...
function StepResults({plan,setPlan,trip,setTrip,onReset,onSaveTrip,savedTrips,debugLog}) {
  ...
  function updatePref(changes) {
    ...
    if (newTrip.routeLegs && (
      ...
      segs = buildSegsFromLegs(newTrip.routeLegs, newTrip.fuelType, newTrip.totalMiles, practicalRange, newTrip.bufferMiles||25, newTrip.maxDetour||2);
    }
...
export default function App() {
  ...
  async function calc(){
    ...
    try {
      ...
      if(route.routeLegs) segs = buildSegsFromLegs(route.routeLegs, vehicle.fuelType, route.totalMiles, practicalRange, prefs.bufferMiles||25, prefs.maxDetour||2, setDebugLog);
