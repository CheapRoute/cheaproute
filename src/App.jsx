...
async function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange, setDebugLog) {
  // Candidate spacing: vehicleRange minus a 60mi arrival buffer.
  // e.g. RX350 (378mi range) → 318mi spacing → ~4-5 candidates on a 1500mi trip.
  const stopInterval = Math.min(350, Math.max(150, (vehicleRange || 300) - 60));

  // Build a detailed point list from the actual Google route geometry.
  // Each point has accurate cumulative miles, lat/lng, AND the state
  // extracted from the step's instruction text — no geocoding API needed.
  const routePoints = [];
  let cumMiles = 0;
  for (const leg of legs) {
    for (const step of leg.steps) {
      ...
      cumMiles += stepMiles;
      // Extract state from instruction HTML, e.g. "Continue on I-40 E in Tennessee"
      const instrText = (step.html_instructions || "").replace(/<[^>]+>/g, " ");
      // Look for 2-letter state code preceded by " in " or " toward " or just standalone
      const stateMatch = instrText.match(/in\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/i);
      // Also try to match highway state codes like "I-40 E/TN" patterns
      const codeMatch = instrText.match(/([A-Z]{2})-\d+/) ||
                        instrText.match(/US-\d+\s*([A-Z]{2})/);
      routePoints.push({
        miles: cumMiles,
  ...
  // Determine state at a given route point by scanning nearby step instructions
  // for US state names and mapping them to abbreviations.
  const STATE_NAMES = {
  ...
  function getStateAtMile(targetMile) {
    // Look at routePoints within ±50mi of target, scan their instructions for state names
    const nearby = routePoints.filter(p => Math.abs(p.miles - targetMile) < 80);
    for (const p of nearby) {
      for (const [name, abbr] of Object.entries(STATE_NAMES)) {
        if (p.instrText.includes(name)) return abbr;
      }
    }
    // Last resort: use the leg's end_address state
    const lastLeg = legs[legs.length - 1];
    return extractState(lastLeg?.end_address || "");
  }

  // Build candidate stops at each interval
  const segs = [];
  const seen = new Map();
  let nextStopMile = stopInterval;

  while (nextStopMile < totalMiles - 30) {
    const thisMile = nextStopMile; // snapshot — prevents closure capture bug

    // Find the route point nearest to this candidate mile
    ...
    // Get state from actual route instructions at this mile position
    const state = getStateAtMile(thisMile);
    // Map state to its best highway fuel stop city
    const city = STATE_HIGHWAY_CITY[state] || `Mile ${Math.round(thisMile)} stop`;
    const source = STATE_HIGHWAY_CITY[state] ? "✓ route+state" : "◆ fallback";

    if (setDebugLog) {
      setDebugLog(prev => [...prev, `Mile ${Math.round(thisMile)}: ${source}: ${city} (${state})`]);
    }
...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    if (mustStop) {
      ...
      if (optimization === "fastest") {
        targetFuel = usableTank; // fill full → fewest stops
      } else if (optimization === "cheapest") {
        // Just enough to reach next stop + generous buffer, no excess
        targetFuel = Math.min(usableTank, minFuelNeeded + (bufferMiles / mpg));
      } else {
        // Balanced: fill to 85% or enough to reach next stop + 80mi, whichever bigger
        targetFuel = Math.min(usableTank, Math.max(minFuelNeeded + 80/mpg, usableTank * 0.85));
      }
  ...
  // Savings = what those same gallons would have cost at the route avg price
  // vs what you actually paid. Ignores starting fuel (sunk cost, same in both scenarios).
  const totalGallonsBought = stops.reduce((s,st)=>s+st.gallons, 0);
  const hypotheticalCost = totalGallonsBought * avgPrice; // same gallons at avg price
  const savings = Math.max(0, hypotheticalCost - optCost);
  const realBaseline = totalFuelNeeded * avgPrice;
  const weightedOptCost = optCost; // what you actually paid at stops
...
function StepVehicle({data,onChange,onNext,onBack,saved,onSave}) {
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      {/* VEHICLE DEBUG PANEL — remove before launch */}
      <div style={{background:"#1a2b3c",borderRadius:12,padding:14,fontSize:11,fontFamily:"monospace",marginBottom:12,color:"#cdd6e8"}}>
        <div style={{color:"#00a878",fontWeight:700,marginBottom:8}}>🔍 VEHICLE DEBUG</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"4px 16px"}}>
          {[
            ["mpg", data.mpg, hasMpg ? "✓" : "✗ MISSING"],
            ["listedTank", data.listedTank, listedNum > 0 ? "✓" : "—"],
            ["usableTank", data.usableTank, usableNum > 0 ? "✓" : "—"],
            ["hasTank", hasTank, hasTank ? "✓" : "✗ MISSING"],
            ["fuelType", data.fuelType, data.fuelType ? "✓" : "✗"],
            ["tankPct", data.tankPct, data.tankPct ? "✓" : "✗"],
            ["make", data.make, data.make ? "✓" : "—"],
            ["model", data.model, data.model ? "✓" : "—"],
            ["ready", String(ready), ready ? "✓ GOOD" : "✗ BLOCKED"],
          ].map(([k,v,status])=>(
            <div key={k} style={{color:status.startsWith("✓")?"#7ee8a2":status.startsWith("✗")?"#ff8a80":"#ffd966"}}>
              <span style={{color:"#8899aa"}}>{k}:</span> {String(v||"—")} <span style={{fontSize:10}}>{status}</span>
            </div>
          ))}
        </div>
        {!ready && <div style={{marginTop:8,color:"#ff8a80",fontWeight:700}}>⚠ Button blocked: {readyHint}</div>}
        {ready && <div style={{marginTop:8,color:"#7ee8a2",fontWeight:700}}>✓ Button should work — if clicking still fails, there is a crash in onSave/onNext</div>}
      </div>
      ...
      <div style={{display:"flex",gap:10}}>
        ...
        {!ready && readyHint && <div style={{textAlign:"center",fontSize:12,color:C.muted,marginTop:6}}>{readyHint}</div>}
        {clickError && (
          <div style={{marginTop:10,background:"#2d0a0a",border:"1.5px solid #d93025",borderRadius:10,padding:14,fontSize:11,fontFamily:"monospace",color:"#ff8a80",whiteSpace:"pre-wrap",wordBreak:"break-all"}}>
            <div style={{color:"#ff5252",fontWeight:700,marginBottom:6}}>💥 CLICK HANDLER CRASH:</div>
            {clickError}
          </div>
        )}
      </div>
    ...
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      {debugLog&&debugLog.length>0&&(
        <div style={{marginTop:16,background:"#1a2b3c",borderRadius:12,padding:16,fontSize:11,fontFamily:"monospace",overflowY:"auto",maxHeight:300}}>
          ...
          {debugLog.map((line,i)=>(
            <div key={i} style={{color:line.includes("✓")?"#7ee8a2":line.includes("◆")?"#ffd966":"#ff8a80",marginBottom:2}}>{line}</div>
          ))}
        </div>
      )}
