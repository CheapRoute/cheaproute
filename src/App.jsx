...
async function buildSegsFromLegs(legs, fuelType, totalMiles) {
  // 120 mi gives ~12 candidate stops on a 1,500-mile trip — 60 was too dense
  // and caused the engine's range tracking to drift badly on skipped stops.
...
function RouteMap({origin,destination,stops,originCoords,destCoords,waypoints}) {
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(maps=>{
      ...
      // Draw route with any user-added waypoints
...
export default function App() {
  ...
  async function calc(){
    setLoading(true);
    let segs=[];
    if(route.routeLegs) segs = await buildSegsFromLegs(route.routeLegs, vehicle.fuelType, route.totalMiles);
