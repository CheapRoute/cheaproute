...
async function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange, setDebugLog) {
  // Candidate spacing = vehicleRange - 60mi so after a full fill
  // the car arrives at the next candidate with ~60mi remaining.
  const stopInterval = Math.min(350, Math.max(150, (vehicleRange || 300) - 60));

  // Build accurate route point list from Google's actual route geometry.
  // Each point has real cumulative miles from the Directions API step data.
  const routePoints = [];
  let cumMiles = 0;
  ...
  // Reverse geocode a lat/lng using the Maps JS Geocoder.
  // Returns "City, ST" or null on failure.
  ...
  // For each candidate mile, find nearest route point and geocode it.
  // Geocode in small batches with delay to respect rate limits.
  const candidates = [];
  ...
  while (nextStopMile < totalMiles - 30) {
    if (routePoints.length > 0) {
      const point = routePoints.reduce((best, p) =>
        ...
        routePoints[0]);
      candidates.push({ mile: nextStopMile, lat: point.lat, lng: point.lng });
    }
    nextStopMile += stopInterval;
  }

  // Geocode all candidates, 2 at a time with 300ms gap to avoid OVER_QUERY_LIMIT
  const segs = [];
  const seen = new Map();
  for (let i = 0; i < candidates.length; i++) {
    const { mile, lat, lng } = candidates[i];
    const thisMile = mile;

    let city = await reverseGeocode(lat, lng);
    const source = city ? "✓ geocoded" : "◆ corridor";
    if (!city) city = getFallbackCity(thisMile, totalMiles, legs);

    if (setDebugLog) {
      const logLine = `Mile ${Math.round(thisMile)}: ${source}: ${city}`;
      setDebugLog(prev => [...prev, logLine]);
    }
    ...
    // Throttle to avoid geocoder rate limit (2 requests/sec max)
    if (i < candidates.length - 1) await new Promise(r => setTimeout(r, 350));
  }
...
function getFallbackCity(targetMile, totalMiles, legs) {
  ...
  const corridorData = CORRIDOR_MILES[corridorKey] || [];
  if (corridorData.length > 0) {
    const closest = corridorData.reduce((best, e) =>
      Math.abs(e[1]-targetMile) < Math.abs(best[1]-targetMile) ? e : best, corridorData[0]);
    return closest[0];
  }
  ...
  if (frac < 0.15) return `${oCity} area, ${oState}`;
  if (frac > 0.85) return `${dCity} area, ${dState}`;
  return `Mile ${Math.round(targetMile)}, ${oState}`;
}
...
function getCityForPoint(lat, lng, state, legs, targetMile, totalMiles) {
  ...
  if (corridorData.length > 0) {
    // Find the corridor city whose real mile position is closest to this stop
    const closest = corridorData.reduce((best, entry) =>
      Math.abs(entry[1] - targetMile) < Math.abs(best[1] - targetMile) ? entry : best,
      corridorData[0]
    );
    return closest[0];
  }

  // Fallback: use state code from GPS step
  const oCity = legs[0].start_address.split(",")[0].trim();
  const dCity = legs[legs.length-1].end_address.split(",")[0].trim();
  const frac = targetMile / totalMiles;
  if (frac < 0.15) return `${oCity} area, ${oState}`;
  if (frac > 0.85) return `${dCity} area, ${dState}`;
  return state && state !== "DEFAULT" ? `${state} Mile ${Math.round(targetMile)}` : `Mile ${Math.round(targetMile)} stop`;
}

// Corridor cities with their REAL approximate mile positions from origin
// Format: [cityName, milesFromOrigin]
const CORRIDOR_MILES = {
  "NC-CO": [
    ["Knoxville, TN", 115],
    ["Nashville, TN", 260],
    ["Memphis, TN", 390],
  ...
  "FL-FL": [
    ...
    ["West Palm Beach, FL", 80],
    ["Vero Beach, FL", 120],
    ["Daytona Beach, FL", 230],
  ],
  "TX-TX": [
    ["Waco, TX", 100],
    ["Dallas, TX", 185],
...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    // Look ahead: find how far we can go before hitting empty
    // If we can't safely reach the next stop after this one, we must stop here
    let mustStop = roa < bufferMiles || roa < 0;
    if (!mustStop) {
      // Check if skipping this stop causes problems down the line
      let lookaheadRange = roa;
      for (let j = i+1; j < Math.min(i+4, segs.length); j++) {
        lookaheadRange -= (segs[j].miles - segs[j-1].miles);
        if (lookaheadRange < bufferMiles) { mustStop = true; break; }
      }
      // Also check final destination
      if (!mustStop && (roa - (totalMiles - seg.miles)) < bufferMiles) mustStop = true;
    }
    const cantReachNext = ranAfterNext < bufferMiles;

    // Never skip if range is critical regardless of detour time
    ...
    if (mustStop) {
      ...
      // Target: enough to reach the next stop + buffer, scaled by optimization mode.
      // "Fastest" tops off to full (fewer stops total).
      // "Cheapest" fills just enough (buy more at cheaper stops ahead).
      // "Balanced/rated" fills to comfortably reach the next stop with headroom.
      // How far to the finish line from THIS stop?
      const distToEnd = totalMiles - seg.miles;
      let targetFuel;
      if (optimization === "fastest") {
        // Fill full unless we can coast to the destination — then buy exactly enough
        const exactToEnd = (distToEnd + bufferMiles) / mpg;
        targetFuel = exactToEnd < usableTank ? Math.min(usableTank, exactToEnd) : usableTank;
      } else if (optimization === "cheapest") {
        // Buy just enough to reach next stop + buffer (save expensive fuel for cheaper stops)
        targetFuel = Math.min(usableTank, minFuelNeeded + bufferMiles / mpg);
      } else {
        // Balanced/rated: fill to reach next stop + 100mi comfort cushion
        // But if we're near the end, just enough to finish + buffer
        const comfortTarget = minFuelNeeded + 100 / mpg;
        const endTarget = (distToEnd + bufferMiles) / mpg;
        targetFuel = Math.min(usableTank, Math.min(comfortTarget, endTarget < usableTank ? endTarget : comfortTarget));
      }
    ...
    if (action==="stop" && gallons > 0) {
      ...
      range = fuel * mpg;
      // milesDone is already set to seg.miles above — no change needed here
    }
  }
  ...
  // Savings = what the trip would cost paying national average price everywhere
  // vs what CheapRoute had you actually pay. Shows real money saved.
  const totalGallonsBought = stops.reduce((s,st)=>s+st.gallons, 0);
  const nationalAvg = BASE_PRICES[fuelType?.toLowerCase()] || 3.35; // national baseline
  const realBaseline = totalFuelNeeded * nationalAvg;
...
function RouteMap({stops,originCoords,destCoords,routeResult,origin,destination}) {
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(maps=>{
      ...
      if (routeResult) {
        // setDirections reuses the exact route from Step 1 — no re-routing.
      ...
      {showPop&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {POPULAR_ROUTES.map((r,i)=>(
            <button key={i} onClick={async()=>{onChange({origin:r.origin,destination:r.dest,stops:[],totalMiles:"",driveTime:"",routeLegs:null,originLat:null,originLng:null,destLat:null,destLng:null});setShowPop(false);const maps=await loadGoogleMaps();const gc=new maps.Geocoder();gc.geocode({address:r.origin},(res,st)=>{if(st==="OK"&&res[0]){const loc=res[0].geometry.location;onChange({originLat:loc.lat(),originLng:loc.lng()});}});gc.geocode({address:r.dest},(res,st)=>{if(st==="OK"&&res[0]){const loc=res[0].geometry.location;onChange({destLat:loc.lat(),destLng:loc.lng()});}});}} style={{background:"#f5f8fc",border:`1.5px solid ${C.border}`,borderRadius:10,padding:"10px 12px",textAlign:"left",color:C.text,fontSize:12}}>
...
// ─── STEP 4: RESULTS ──────────────────────────────────────────────────────────
function StepResults({plan,setPlan,trip,setTrip,onReset,onSaveTrip,savedTrips,debugLog}) {
    ...
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <Card style={{marginBottom:16,background:"#f8fafd"}}>
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>updatePref({bufferMiles:Math.min(trip.bufferMiles+20,100),maxDetour:Math.min(trip.maxDetour+3,15)})} style={{flex:1,background:"#f0ebff",border:"1.5px solid #c4b5fd",color:C.purple,borderRadius:8,padding:"9px 8px",fontSize:13,fontWeight:600}}>⬇ Fewer Stops</button>
          <button onClick={()=>updatePref({bufferMiles:Math.max(trip.bufferMiles-20,25),maxDetour:Math.max(trip.maxDetour-3,2)})} style={{flex:1,background:"#fff7ed",border:"1.5px solid #fcd9a0",color:C.amber,borderRadius:8,padding:"9px 8px",fontSize:13,fontWeight:600}}>⬆ More Stops</button>
      ...
      {debugLog&&debugLog.length>0&&(
        <div style={{marginTop:16,background:"#1a2b3c",borderRadius:12,padding:16,fontSize:11,fontFamily:"monospace",overflowY:"auto",maxHeight:300}}>
          <div style={{color:"#00a878",fontWeight:700,marginBottom:8,fontSize:12}}>🔍 GEOCODER DEBUG — screenshot this!</div>
          {debugLog.map((line,i)=>(
            <div key={i} style={{color:line.includes("✓ geocoded")?"#7ee8a2":line.includes("◆ corridor")?"#ffd966":"#ff8a80",marginBottom:2}}>{line}</div>
          ))}
        </div>
      )}
...
export default function App() {
  ...
  async function calc(){
    ...
    if(route.routeLegs) segs = await buildSegsFromLegs(route.routeLegs, vehicle.fuelType, route.totalMiles, practicalRange, setDebugLog);
    ...
    setTrip(t);setPlan(runEngine(t,segs));setStep(3);setLoading(false);
  }
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:C.bg}}>
        ...
        <div style={{maxWidth:640,margin:"0 auto",padding:"28px 18px 80px",minHeight:"calc(100vh - 62px)"}}>
          ...
          {step===3&&plan&&trip&&<StepResults plan={plan} setPlan={setPlan} trip={trip} setTrip={setTrip} onReset={reset} onSaveTrip={saveTrip} savedTrips={savedTrips} debugLog={debugLog} />}
        </div>
