...
function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange, setDebugLog) {
  // Candidate spacing: vehicleRange - 60mi so car arrives with ~60mi buffer
  const stopInterval = Math.min(350, Math.max(150, (vehicleRange || 300) - 60));

  // Build accurate point list from Google's actual route geometry
  const routePoints = [];
  let cumMiles = 0;
  ...
  // Determine US state from lat/lng using bounding boxes — no API needed
  // Boxes are [minLat, maxLat, minLng, maxLng]
  const STATE_BOXES = {
    AL:[30.14,35.01,-88.47,-84.89], AR:[33.00,36.50,-94.62,-89.64],
    AZ:[31.33,37.00,-114.82,-109.04], CA:[32.53,42.01,-124.41,-114.13],
    CO:[36.99,41.00,-109.05,-102.04], CT:[40.95,42.05,-73.73,-71.79],
    DE:[38.45,39.84,-75.79,-74.98], FL:[24.52,31.00,-87.63,-80.03],
    GA:[30.36,35.00,-85.61,-80.84], IA:[40.38,43.50,-96.64,-90.14],
    ID:[41.99,49.00,-117.24,-111.04], IL:[36.97,42.51,-91.51,-87.49],
    IN:[37.77,41.76,-88.10,-84.78], KS:[36.99,40.00,-102.05,-94.59],
    KY:[36.50,39.15,-89.57,-81.96], LA:[28.93,33.02,-94.04,-88.82],
    MA:[41.24,42.89,-73.50,-69.93], MD:[37.91,39.72,-79.49,-75.05],
    ME:[43.06,47.46,-71.08,-66.95], MI:[41.70,48.19,-90.42,-82.41],
    MN:[43.50,49.38,-97.24,-89.49], MO:[35.99,40.61,-95.77,-89.10],
    MS:[30.17,35.00,-91.66,-88.10], MT:[44.36,49.00,-116.05,-104.04],
    NC:[33.84,36.59,-84.32,-75.46], ND:[45.94,49.00,-104.05,-96.55],
    NE:[40.00,43.00,-104.05,-95.31], NH:[42.70,45.31,-72.56,-70.61],
    NJ:[38.93,41.36,-75.56,-73.90], NM:[31.33,37.00,-109.05,-103.00],
    NV:[35.00,42.00,-120.00,-114.04], NY:[40.50,45.01,-79.76,-71.86],
    OH:[38.40,42.33,-84.82,-80.52], OK:[33.62,37.00,-103.00,-94.43],
    OR:[41.99,46.24,-124.57,-116.46], PA:[39.72,42.27,-80.52,-74.69],
    RI:[41.15,42.02,-71.91,-71.12], SC:[32.05,35.22,-83.35,-78.54],
    SD:[42.48,45.94,-104.06,-96.44], TN:[34.98,36.68,-90.31,-81.65],
    TX:[25.84,36.50,-106.65,-93.51], UT:[36.99,42.00,-114.05,-109.04],
    VA:[36.54,39.47,-83.68,-75.24], VT:[42.73,45.02,-73.44,-71.50],
    WA:[45.54,49.00,-124.73,-116.92], WI:[42.49,47.08,-92.89,-86.25],
    WV:[37.20,40.64,-82.64,-77.72], WY:[40.99,45.01,-111.06,-104.05],
  };

  function getStateFromLatLng(lat, lng) {
    for (const [state, [minLat, maxLat, minLng, maxLng]] of Object.entries(STATE_BOXES)) {
      if (lat >= minLat && lat <= maxLat && lng >= minLng && lng <= maxLng) return state;
    }
    return null;
  }
  ...
  while (nextStopMile < totalMiles - 30) {
    ...
    const state = getStateFromLatLng(point.lat, point.lng);
    const city = (state && STATE_HIGHWAY_CITY[state]) || getFallbackCity(thisMile, totalMiles, legs);
    const source = (state && STATE_HIGHWAY_CITY[state]) ? `✓ lat/lng→${state}` : "◆ fallback";

    if (setDebugLog) {
      setDebugLog(prev => [...prev, `Mile ${Math.round(thisMile)}: ${source}: ${city}`]);
    }
...
export default function App() {
  ...
  async function calc(){
    ...
    if(route.routeLegs) segs = buildSegsFromLegs(route.routeLegs, vehicle.fuelType, route.totalMiles, practicalRange, setDebugLog);
