...
async function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange) {
  // Space candidate stops at ~60% of practical vehicle range so the engine
  // always has 1-2 candidates per tank, never dozens. Clamped 100–250 mi.
  const stopInterval = Math.min(250, Math.max(100, Math.round((vehicleRange || 300) * 0.60)));
  const routePoints = [];
  let cumMiles = 0;
...
function RouteMap({origin,destination,stops,originCoords,destCoords}) {
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(maps=>{
      ...
      // Use exact lat/lng for origin & dest so Google doesn't re-snap the route.
      // Use fuel stop coords as via-waypoints (stopover:false = pass-through, no U-turn).
      const fuelWps = (stops||[])
        .filter(s=>s.lat&&s.lng&&!s.skipped)
        .map(s=>({location:new maps.LatLng(s.lat,s.lng),stopover:false}))
        .slice(0,23);
