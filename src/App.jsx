...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    // Mode-specific skip logic — when we have range to spare, should we stop here?
    const canSkipSafely = !mustStop && !cantReachNext;
    if (canSkipSafely && optimization !== "fastest") {
      const pricePremium = priceHere - priceNext;
      if (optimization === "cheapest" && pricePremium > 0.02) continue;
      if (optimization === "balanced" && pricePremium > 0.06) continue;
