...
function buildSegsFromLegs(legs, fuelType, totalMiles) {
  // Space stops every ~150 miles (well within any vehicle's range)
  // This ensures the engine always has decision points before fuel runs low
  const spacing = 150;
  const count = Math.max(3, Math.min(14, Math.floor(totalMiles / spacing)));
  const segs = [];
  ...
  // Get multi-leg cities if available
  const legCities = legs.length > 1 ? legs.map(leg => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const stopMiles = Math.round(frac * totalMiles);
    let city;
    ...
    if (legCities.length > 1) {
      ...
      // Map this stop's mile marker to the corridor city list
      const idx = Math.min(Math.floor(frac * corridorCities.length), corridorCities.length - 1);
      city = corridorCities[idx];
    } else {
      city = frac < 0.33 ? `${oCity} area, ${oState}` : frac < 0.66 ? `${oState} Midpoint, ${oState}` : `${dCity} area, ${dState}`;
    }
