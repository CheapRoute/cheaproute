...
function RouteMap({origin,destination,stops,originCoords,destCoords,waypoints}) {
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(maps=>{
      ...
      markersRef.current=[];
      // Always recreate renderer to ensure route updates correctly
      ...
      // Draw route through the actual fuel stops (lat/lng from geocoding),
      // not the user-entered waypoint text which belongs to a different leg.
      // Google Directions API allows max 25 waypoints; sample if needed.
      const fuelWps = (stops||[])
        .filter(s=>s.lat&&s.lng&&!s.skipped)
        .map(s=>({location:{lat:s.lat,lng:s.lng},stopover:false}))
        .slice(0,23); // API max is 25 incl. origin/dest
      new maps.DirectionsService().route({origin,destination,waypoints:fuelWps,travelMode:maps.TravelMode.DRIVING,optimizeWaypoints:false},(r,s)=>{
