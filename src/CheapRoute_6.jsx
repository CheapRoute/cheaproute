...
function StepVehicle({data,onChange,onNext,onBack,saved,onSave}) {
  ...
  async function pickTrim(opt) {
    ...
    if (specs) {
      ...
      // Build the complete vehicle state update
      const listedStr = listedTank ? String(listedTank) : data.listedTank||"";
      const usableStr = data.tankLocked ? data.usableTank : usableAtEmpty;
      onChange({
        trim: opt.text,
