...
function runEngine(trip, segsOverride) {
  ...
  for (let i=0; i<segs.length; i++) {
    ...
    // Mode-specific skip logic — skip this stop if we have range AND a reason to
    const canSkipSafely = !mustStop && !cantReachNext;
    if (canSkipSafely) {
      if (optimization === "fastest") {
        // Fastest: only stop when absolutely forced (mustStop/cantReachNext already handled)
        // If we can skip safely, always skip — fewest stops possible
        continue;
      }
      if (optimization === "cheapest") {
        // Cheapest: skip if price here is higher than OR equal to next stop
        // Only stop if price here is genuinely cheaper
        if (!cheaperHere) continue;
      }
      if (optimization === "balanced") {
        // Balanced: skip if price here is slightly higher than next
        if (priceHere > priceNext * 0.99) continue;
      }
      // toprated: never skip when we can stop — always consider it
    }

    // Smart stop: worthwhile to stop even though we don't HAVE to
    // Each mode has different thresholds for what counts as "worth it"
    const smartStop =
      optimization === "cheapest"  ? (priceNext > priceHere*1.05 && roa > bufferMiles) :            // any price rise
      ...
      optimization === "toprated"  ? (seg.rating >= 4.0 && roa > bufferMiles*2) : // high-rated station
                                     (priceNext > priceHere*1.05 && roa > bufferMiles * 1.5);         // balanced: moderate rise
    ...
    } else if (priceNext < priceHere*0.97 && ranAfterNext > bufferMiles * 1.5) {
