...
function buildSegsFromLegs(legs, fuelType, totalMiles) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const legIdx = Math.min(Math.floor(frac * legs.length), legs.length - 1);
    const leg = legs[legIdx];
    const city = leg.end_address.split(",").slice(0, 2).join(",").trim();
    segs.push({
      city, miles: Math.round(frac * totalMiles),
...
async function fetchMakes(year) {
  try {
    const r = await fetch(`https://vpic.nhtsa.dot.gov/api/vehicles/GetMakesForVehicleType/car?year=${year}&format=json`);
    const d = await r.json();
    return (d.Results||[]).map(m=>m.MakeName).sort();
  } catch { return []; }
}
...
async function fetchEpaTrims(year, make, model) {
  // Try exact model name first, then try searching EPA vehicle list
  async function tryModel(m) {
    try {
      const r = await fetch(`https://www.fueleconomy.gov/ws/rest/vehicle/menu/options?year=${year}&make=${encodeURIComponent(make)}&model=${encodeURIComponent(m)}`,{headers:{Accept:"application/json"}});
      const d = await r.json();
      return Array.isArray(d.menuItem)?d.menuItem:d.menuItem?[d.menuItem]:[];
    } catch { return []; }
  }
  // Try exact match first
  let results = await tryModel(model);
  if (results.length) return results;
  // Try searching EPA's full vehicle list for this year/make to find matching models
  try {
    const r = await fetch(`https://www.fueleconomy.gov/ws/rest/vehicle/menu/model?year=${year}&make=${encodeURIComponent(make)}`,{headers:{Accept:"application/json"}});
    const d = await r.json();
    const epaModels = Array.isArray(d.menuItem)?d.menuItem:d.menuItem?[d.menuItem]:[];
    // Find EPA model that best matches what user typed (case-insensitive, partial match)
    const userModel = model.toLowerCase().replace(/[-\s]/g,'');
    const match = epaModels.find(m => {
      const epa = m.value.toLowerCase().replace(/[-\s]/g,'');
      return epa.includes(userModel) || userModel.includes(epa) || epa.startsWith(userModel.slice(0,3));
    });
    if (match) {
      results = await tryModel(match.value);
      if (results.length) return results;
    }
    // Return all EPA models for this make so user can see options
    return epaModels.map(m=>({text:m.value, value:m.value, isModel:true}));
  } catch { return []; }
}
async function fetchEpaById(id) {
  try {
    ...
    return {
      ...
      fuelType:d.fuelType1?.includes("Premium")?"Premium":d.fuelType1?.includes("Diesel")?"Diesel":"Regular"
    };
  } catch { return null; }
}
...
// ─── MAP COMPONENT ────────────────────────────────────────────────────────────
function RouteMap({origin,destination,stops,originCoords,destCoords}) {
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(maps=>{
      ...
      // Draw route
      new maps.DirectionsService().route({origin,destination,travelMode:maps.TravelMode.DRIVING},(r,s)=>{
  ...
  },[origin,destination,stops,originCoords,destCoords]);
  ...
  async function calcDistance() {
    ...
    try {
      ...
      const result = await new Promise((res,rej)=>{
        new maps.DirectionsService().route({origin:data.origin,destination:data.destination,travelMode:maps.TravelMode.DRIVING},(r,s)=>s==="OK"?res(r):rej(s));
      });
    ...
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      {data.origin&&data.destination&&!data.totalMiles&&(
        <button onClick={calcDistance} disabled={loading} style={{width:"100%",marginBottom:16,padding:"12px",background:C.blueDim,border:`1.5px solid ${C.blueBorder}`,color:C.blueMid,borderRadius:10,fontSize:14,fontWeight:600,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          {loading?<><Spinner sm/>&nbsp;Calculating exact route...</>:"📍 Get exact miles & drive time from Google Maps"}
        </button>
      )}
      ...
      {showPop&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
          {POPULAR_ROUTES.map((r,i)=>(
            <button key={i} onClick={()=>{onChange({origin:r.origin,destination:r.dest,totalMiles:"",driveTime:"",routeLegs:null});setShowPop(false);}} style={{background:"#f5f8fc",border:`1.5px solid ${C.border}`,borderRadius:10,padding:"10px 12px",textAlign:"left",color:C.text,fontSize:12}}>
      ...
      {data.totalMiles&&(
        <Card accent style={{marginBottom:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12}}>
            ...
            <Stat label="Est. Stops" value={`${Math.max(0,Math.floor(data.totalMiles/280))}–${Math.ceil(data.totalMiles/180)}`} />
          </div>
        </Card>
      )}
...
function StepVehicle({data,onChange,onNext,onBack,saved,onSave}) {
  ...
  useEffect(()=>{
    if (!data.year||!data.make||!data.model){setTrims([]);setEpaStatus("idle");return;}
    setEpaStatus("loading");
    fetchEpaTrims(data.year,data.make,data.model).then(opts=>{
      setTrims(opts);
  ...
  async function pickTrim(opt) {
    // If this is a model-level result, fetch its trims first
    if (opt.isModel) {
      setEpaStatus("loading");
      const subTrims = await fetchEpaTrims(data.year, data.make, opt.value);
      setTrims(subTrims.filter(t=>!t.isModel));
      setEpaStatus(subTrims.filter(t=>!t.isModel).length?"options":"notfound");
      return;
    }
    ...
    if (specs) {
      const suggestedMpg = String(Math.round(specs.combined*REAL_WORLD_FACTOR*10)/10);
      onChange({
        trim:opt.text,
        epaCombined:specs.combined, epaCity:specs.city, epaHwy:specs.hwy,
        fuelType:specs.fuelType,
        mpg:data.mpgLocked?data.mpg:suggestedMpg,
        listedTank:data.listedTank||"",
      });
      setEpaStatus("found");
    } else { setEpaStatus("notfound"); }
  }

  const mpg=parseFloat(data.mpg)||0, usable=parseFloat(data.usableTank)||0, listed=parseFloat(data.listedTank)||0;
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <p style={{color:C.muted,fontSize:14,marginBottom:20}}>Makes & models from NHTSA database · MPG auto-fills from EPA</p>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:4}}>
        ...
        <div>
          <Lbl hint={makesLoading?"Loading from NHTSA...":""}>Make</Lbl>
      ...
      <div style={{marginBottom:16}}>
        <Lbl hint={modelsLoading?"Loading models...":""}>Model</Lbl>
        <select value={data.model||""} onChange={e=>onChange({model:e.target.value,trim:""})} disabled={!models.length}>
          <option value="">{modelsLoading?"Loading...":models.length?"Select model":"Select make first"}</option>
      ...
      <div style={{background:"#f8fafd",border:`1.5px solid ${C.border}`,borderRadius:12,padding:16,marginBottom:16}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <div>
            ...
            {data.epaCombined&&<div style={{fontSize:11,color:C.hint,marginTop:4}}>EPA combined: {data.epaCombined} MPG</div>}
          </div>
          <div>
            <Lbl>Listed Tank Size</Lbl>
            <div style={{position:"relative"}}>
              <input type="number" value={data.listedTank||""} onChange={e=>onChange({listedTank:e.target.value})} placeholder="19.2" style={{paddingRight:36}} />
          ...
          <div>
            <Lbl hint="How much you fill when the gauge hits E">Gallons at Empty</Lbl>
            <div style={{position:"relative"}}>
              <input type="number" value={data.usableTank||""} onChange={e=>onChange({usableTank:e.target.value})} placeholder="16.5" style={{paddingRight:36}} />
              <span style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",fontSize:12,color:C.muted,pointerEvents:"none"}}>gal</span>
            </div>
          </div>
        </div>
        {data.listedTank&&!data.usableTank&&(
          <button onClick={()=>onChange({usableTank:String(Math.round(parseFloat(data.listedTank)*USABLE_TANK_FACTOR*10)/10)})} style={{background:"none",border:`1px solid ${C.border}`,color:C.muted,borderRadius:8,padding:"6px 12px",fontSize:12,marginTop:8}}>
            ↺ Auto-estimate gallons at empty
          </button>
        )}
      </div>
...
function StepResults({plan,setPlan,trip,setTrip,onReset,onSaveTrip,savedTrips}) {
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      {view==="map"&&(
        <div style={{marginBottom:16}}>
          <RouteMap origin={trip.origin} destination={trip.destination} stops={stops} originCoords={originCoords} destCoords={destCoords} />
...
// ─── APP ROOT ─────────────────────────────────────────────────────────────────
const defRoute={origin:"",destination:"",totalMiles:"",driveTime:"",originLat:null,originLng:null,destLat:null,destLng:null,routeLegs:null};
