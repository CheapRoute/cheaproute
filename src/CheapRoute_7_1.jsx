...
async function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange, setDebugLog) {
  // Space candidates at (vehicleRange - 60mi buffer) so after a full fill
  // the car arrives at the next candidate with ~60mi to spare.
  // Clamp 150–350 mi so short-range and long-range cars both work.
  const stopInterval = Math.min(350, Math.max(150, (vehicleRange || 300) - 60));

  // Build route point list for lat/lng lookup at each candidate mile.
  const routePoints = [];
  let cumMiles = 0;
  ...
  // - Primary: corridor lookup (reliable, accurate, no API calls)
  // - Fallback: generic mile marker
  // We do NOT use leg.start_address — with a direct route (no user waypoints)
  // there is only 1 leg, so every candidate would return the same origin city,
  // which deduplication would then collapse into a single stop.
  ...
  while (nextStopMile < totalMiles - 30) {
    // Snapshot mile NOW before any async state updates (closure bug prevention)
    ...
    const city = getFallbackCity(thisMile, totalMiles, legs);
    const cityKey = city.toLowerCase().trim();

    if (setDebugLog) {
      const logLine = `Mile ${Math.round(thisMile)}: ◆ corridor: ${city}`;
