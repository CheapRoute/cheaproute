...
async function fetchEpaById(id) {
  try {
    ...
    return {
      ...
      tank: parseFloat(d.lv2)||parseFloat(d.lv4)||0,
      fuelType: d.fuelType1?.includes("Premium")?"Premium":d.fuelType1?.includes("Diesel")?"Diesel":d.fuelType1?.includes("Midgrade")?"Midgrade":"Regular"
    };
  } catch { return null; }
}
...
function StepVehicle({data,onChange,onNext,onBack,saved,onSave}) {
  ...
  useEffect(()=>{
    ...
    // EPA model lookup — perfectly consistent with EPA trim lookup
  ...
  async function pickTrim(opt) {
    ...
    if (specs) {
      ...
      const suggestedTank = String(Math.round((specs.tank||0)*USABLE_TANK_FACTOR*10)/10);
      onChange({
        trim: opt.text,
        epaCombined: specs.combined, epaCity: specs.city, epaHwy: specs.hwy,
        fuelType: specs.fuelType,
        mpg: data.mpgLocked ? data.mpg : suggestedMpg,
        listedTank: specs.tank ? String(specs.tank) : data.listedTank||"",
        usableTank: data.tankLocked ? data.usableTank : (specs.tank ? suggestedTank : data.usableTank||""),
      });
      setEpaStatus("found");
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      <div style={{marginBottom:16}}>
        <Lbl hint={modelsLoading?"Loading from EPA...":models.length?`${models.length} models available`:"Select make first"}>Model</Lbl>
