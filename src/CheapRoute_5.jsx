...
async function buildSegsFromLegs(legs, fuelType, totalMiles, vehicleRange, setDebugLog) {
  // Place candidate stops every 60 miles so the engine always has
  // plenty of decision points regardless of vehicle range
  ...
  // Collect all stop positions first
  const stopPositions = [];
  ...
  while (nextStopMile < totalMiles - 30) {
    ...
    if (point) stopPositions.push({...point, targetMile: nextStopMile});
    nextStopMile += stopInterval;
  }

  // Reverse geocode all stops in parallel using Google Geocoder
    ...
    return new Promise(resolve => {
      geocoder.geocode({location:{lat, lng}}, (results, status) => {
        if (status === "OK" && results && results.length > 0) {
          // Look through ALL results to find the best locality (city)
          // Highway results often come first — skip them
          let city = "", state = "";
          for (const result of results) {
            ...
            if (locality && st) {
              city = locality;
              state = st;
              break; // Found a proper city — use it
            }
            // Fallback: use county if no city found yet
            if (!city) {
              const county = comps.find(c=>c.types.includes("administrative_area_level_2"))?.long_name || "";
              if (county && st) { city = county.replace(" County",""); state = st; }
            }
          }
          resolve(city && state ? `${city}, ${state}` : null);
        } else {
          resolve(null);
        }
      });
  ...
  const segs = [];
  for (let i = 0; i < stopPositions.length; i += 5) {
    const batch = stopPositions.slice(i, i+5);
    const cities = await Promise.all(batch.map(p => reverseGeocode(p.lat, p.lng)));
    for (let j = 0; j < batch.length; j++) {
      const p = batch[j];
      const geocodedCity = cities[j];
      const fallbackCity = getFallbackCity(p.targetMile, totalMiles, legs);
      const city = geocodedCity || fallbackCity;
      if (setDebugLog) setDebugLog(prev => [...prev, `Mile ${Math.round(p.targetMile)}: ${geocodedCity ? "✓ "+geocodedCity : "✗ fallback: "+fallbackCity}`]);
      segs.push({
        city,
        miles: Math.round(p.targetMile),
        lat: p.lat,
        lng: p.lng,
    ...
    if (i + 5 < stopPositions.length) await new Promise(r => setTimeout(r, 100));
  }

  // Deduplicate: if two candidate stops geocoded to the same city,
  // keep the one whose targetMile is closest to that city's true position.
  const seen = new Map();
  for (const seg of segs) {
    const key = seg.city.toLowerCase().trim();
    if (!seen.has(key)) {
      seen.set(key, seg);
    } else {
      // Keep whichever is closer to the actual midpoint between its neighbours
      // — here we simply keep the later one since it's further along the route
      // and more likely to be a genuine stop location.
      seen.set(key, seg);
    }
  }
...
// ─── MAP COMPONENT ────────────────────────────────────────────────────────────
function RouteMap({origin,destination,stops,originCoords,destCoords,trip}) {
  ...
  useEffect(()=>{
    ...
    loadGoogleMaps().then(maps=>{
      // Always recreate map to ensure correct center for new routes
      ...
      // Draw the direct route — just origin to destination
      // Fuel stop pins are shown as markers, not waypoints
      new maps.DirectionsService().route({
        origin: new maps.LatLng(originCoords.lat, originCoords.lng),
        destination: new maps.LatLng(destCoords.lat, destCoords.lng),
        waypoints: (trip?.stops||[]).filter(s=>s.value).map(s=>({location:s.value,stopover:true})),
  ...
  },[origin,destination,stops,originCoords,destCoords,trip]);
...
function StepResults({plan,setPlan,trip,setTrip,onReset,onSaveTrip,savedTrips,debugLog}) {
  ...
  return (
    <div style={{animation:"fadeIn 0.3s ease"}}>
      ...
      {view==="map"&&(
        <div style={{marginBottom:16}}>
          <RouteMap origin={trip.origin} destination={trip.destination} stops={stops} originCoords={originCoords} destCoords={destCoords} trip={trip} />
      ...
      {debugLog&&debugLog.length>0&&(
        <div style={{marginTop:16,background:"#1a2b3c",borderRadius:12,padding:16,fontSize:11,fontFamily:"monospace"}}>
          <div style={{color:"#00a878",fontWeight:700,marginBottom:8,fontSize:12}}>🔍 GEOCODER DEBUG — screenshot this!</div>
          {debugLog.map((line,i)=>(
            <div key={i} style={{color:line.includes("✓")?"#5ee8c0":"#ffb347",marginBottom:3}}>{line}</div>
          ))}
        </div>
      )}
